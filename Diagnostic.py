    ...
    if mono_mod is not None:
        try:
            ...
            if record.get("Fetch_Error"):
                ...
                mono_report.append((
                    "  -> contraindications_section_status correctly reports "
                    "'has_content_see_raw_text' (Melissa's contraindications "
                    "section has Traditional-use content; this field cannot "
                    "and does not claim to know that WEU specifically is "
                    "empty — read raw_text for that)",
                    "OK" if record.get("contraindications_section_status") == "has_content_see_raw_text"
                    else "STILL WRONG",
                    repr(record.get("contraindications_section_status")),
                ))
                mono_report.append((
