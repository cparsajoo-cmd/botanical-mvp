...
BASE=ROOT/'gold_corpus'/'decision_holdout_v4'
def load_reference(): return json.loads((BASE/'frozen_reference_labels.json').read_text())
def run_case(case):
...
def evaluate():
    ...
    for c in refs:
        actual,row=run_case(c); expected=FinalDecisionStatus(c['expected']); comps.append(DecisionComparison(c['case_id'],expected,actual,expected==actual)); rows.append({'case_id':c['case_id'],'expected':expected.value,'actual':actual.value,'match':expected==actual,'score':float(row.get('Botanical_RD_Score',0))})
    return rows,compute_metrics(comps)
if __name__=='__main__':
    rows,metrics=evaluate()
    metrics_dict=asdict(metrics)
    payload={'version':'decision-holdout-v4/1.0.0','engine_version':DECISION_ENGINE_VERSION,'rows':rows,'metrics':metrics_dict}
    ...
    artifact,registry,_=persist_validation_run(
        repo_root=ROOT, dataset_name='decision_holdout_v4', dataset_version='1.0.0',
        ...
        results_previously_inspected=True, used_for_remediation=None,
        ...
        safety_regulatory_metrics=high_risk, historical_blind_result_path='gold_corpus/decision_holdout_v4/results.json',
