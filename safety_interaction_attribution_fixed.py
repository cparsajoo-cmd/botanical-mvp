...
def _coerce_structured(value: object) -> object:
    """Recover JSON/JSONB containers without flattening their schema."""
    if value is None:
        return None
...
def _render_adverse_object(item: dict) -> str:
    """Render the persisted adverse-event schema without inventing labels."""
    event = next((str(item.get(k)).strip() for k in ("event", "adverse_event", "term", "label", "name")
                  if item.get(k) not in (None, "")), "")
    if not event:
        # Backward-compatible coded maps such as {"bleeding": "reported concern"}.
        # This function is called only for the adverse_events field, so the key
        # itself is the adverse-event term; unrelated indication metadata is
        # never passed here.
        if len(item) == 1:
            key, value = next(iter(item.items()))
            key_text = str(key).strip()
            value_text = str(value).strip() if value not in (None, "") else ""
            return f"{key_text}: {value_text}" if key_text and value_text else key_text
        return ""
    details = []
    for key, label in (("severity", "severity"), ("attribution", "attribution"),
                       ("evidence_type", "evidence"), ("frequency", "frequency")):
        value = item.get(key)
        if value not in (None, ""):
            details.append(f"{label}: {value}")
    return f"{event} ({'; '.join(details)})" if details else event


def _render_interaction_object(item: dict) -> str:
    """Render the persisted interaction schema using its real field names."""
    drug_class = next((str(item.get(k)).strip() for k in (
        "interacting_drug_class", "drug_class", "interaction", "term", "label", "name"
    ) if item.get(k) not in (None, "")), "")
    agents_raw = item.get("agents") or item.get("drugs") or item.get("medications")
    if isinstance(agents_raw, (list, tuple, set)):
        agents = ", ".join(str(v).strip() for v in agents_raw if str(v).strip())
    else:
        agents = str(agents_raw or "").strip()
    heading = drug_class
    if agents:
        heading = f"{heading} — agents: {agents}" if heading else f"agents: {agents}"
    if not heading:
        return ""
    details = []
    for key, label in (("possible_mechanism", "mechanism"), ("mechanism", "mechanism"),
                       ("clinical_consequence", "clinical consequence"),
                       ("attribution", "attribution"), ("evidence_type", "evidence")):
        value = item.get(key)
        if value not in (None, "") and f"{label}: {value}" not in details:
            details.append(f"{label}: {value}")
    return f"{heading} ({'; '.join(details)})" if details else heading
...
def extract_structured_safety_interactions(
    ...
    """Interpret persisted safety JSONB while preserving its schema.

    Structured adverse-event and interaction fields are authoritative about the
    relationship type.  Raw prose is still screened conservatively, but JSONB
    objects are rendered from explicit keys such as ``event``, ``agents`` and
    ``interacting_drug_class``.  Indication fields are never consulted here.
    """
    adverse: list[str] = []
    reassurance: list[str] = []

    for item in _as_sequence(safety_value):
        if isinstance(item, dict):
            rendered = _render_adverse_object(item)
            if rendered:
                adverse.append(rendered)
            continue
        value = str(item).strip()
        if not value or _is_noise(value) or _is_protective_or_negated(value):
            continue
        if _matches_any(value, _REASSURANCE_PATTERNS) or "no important safety concerns" in _norm(value):
            reassurance.append(value)
        elif _is_adverse_statement(value):
            adverse.append(value)
        else:
            n = _norm(value)
            coded_terms = ("bleeding", "hemorrhage", "hyphema", "seizure", "hepatotoxic",
                           "liver injury", "nausea", "vomiting", "diarrhea", "diarrhoea",
                           "rash", "anaphylaxis", "allergic reaction", "toxicity")
            if any(term in n for term in coded_terms):
                adverse.append(value)

    interactions: list[str] = []
    for item in _as_sequence(interaction_value):
        if isinstance(item, dict):
            rendered = _render_interaction_object(item)
            if rendered:
