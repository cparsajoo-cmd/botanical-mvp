...
from scientific_intent_service import parse_scientific_intent
...
def render_inputs():
    ...
    with st.expander("✍️ Describe the project in your own words"):
        ...
        if st.button("Parse this question", key="rd_parse_free_text_btn"):
            # AI scientific-intent understanding, with a mandatory,
            # automatic fallback to the original deterministic keyword
            # parser (parse_free_text_question) on any AI failure --
            # see scientific_intent_service.py. The returned object has
            # the exact same field names/contract as the deterministic
            # ParsedQuestion, so nothing below this line needed to change.
            parsed = parse_scientific_intent(free_text_question)
        ...
        if parsed_question is not None and free_text_question:
            ...
            if found:
                recognition_source = (
                    "AI scientific-intent understanding"
                    if getattr(parsed_question, "ai_used", False)
                    else "deterministic keyword matching (AI unavailable or inconclusive)"
                )
                st.success(
                    f"Recognized from your question (via {recognition_source}):\n\n"
    ...
    standardized_project = standardize_project_definition(
        ...
        # Additive only -- see question_understanding_engine.py. None
        # when the free-text box was never used or the AI path did not
        # run, which keeps standardized_project byte-identical to
        # before this parameter existed.
        ai_intent=getattr(parsed_question, "structured_intent", None),
    )
