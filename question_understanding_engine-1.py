...
def standardize_project_definition(
    ...
    standardized_project = {
        ...
        # Additive, optional AI scientific-intent enrichment (hybrid AI-R&D
        # architecture). ``ai_intent`` is the caller-supplied structured
        # intent from scientific_intent_service.py (already validated,
        # normalized, and fallback-safe there -- this function does not
        # re-validate it, only carries it through). Defaults to None so
        # every existing caller that omits this parameter gets byte-
        # identical output to before this field existed.
        "ai_scientific_intent": ai_intent if isinstance(ai_intent, dict) else None,
    }
