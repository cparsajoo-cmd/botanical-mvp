...
def _pick_raw_from_row(row: pd.Series, names: list[str]) -> object:
    """Return the first non-missing cell without stringifying JSONB values."""
    for name in names:
...
def _aggregate_plant_safety(records: list[dict], plant_name: str) -> dict:
    ...
    for record in records:
        structured_safety = record.get("adverse_events")
        structured_interactions = record.get("interactions_structured")
        safety_findings_text = record.get("safety_findings_text")
        interaction_text = record.get("interaction_text")

        if structured_safety is not None or structured_interactions is not None:
            structured = extract_structured_safety_interactions(
                structured_safety, structured_interactions, plant_name=plant_name,
            )
        ...
        # Safety-findings prose can carry reassurance; interaction text can
        # carry concise structured labels from older rows.  Keep it separate
        # from JSONB so indication/other metadata can never be mis-rendered.
        text_structured = extract_structured_safety_interactions(
            safety_findings_text, interaction_text, plant_name=plant_name,
        )
        adverse_values.extend(text_structured.get("adverse_events", []))
        interaction_values.extend(text_structured.get("interactions", []))
        reassurance_values.extend(text_structured.get("safety_reassurance", []))
    ...
    for frame in frames:
        ...
        for idx, row in frame.iterrows():
            ...
            index.setdefault(plant_key, []).append({
                ...
                # Preserve JSONB containers as objects.  The legacy engine._pick
                # stringifies every value, which destroys the adverse-event and
                # interaction schemas before the renderer can read keys such as
                # event, agents and possible_mechanism.
                "adverse_events": _pick_raw_from_row(row, ["Adverse_Events", "adverse_events"]),
                "interactions_structured": _pick_raw_from_row(row, [
                    "Interactions_Structured", "interactions_structured",
                ]),
                "safety_findings_text": _pick_raw_from_row(row, [
                    "Safety_Findings", "safety_findings", "Safety_Signal", "safety_signal",
                    "Safety_Flags", "safety_flags",
                ]),
                "interaction_text": _pick_raw_from_row(row, [
                    "Interactions", "interactions", "Drug_Interaction_Level",
                    "drug_interaction_level", "Interaction_Flags", "interaction_flags",
                ]),
...
def discover_indication_candidates(engine, indication: str, dosage_form: str = "", market: str = "", product_type: str = "") -> pd.DataFrame:
    ...
    for _, item in candidates.iterrows():
        ...
        for record in evidence_units:
            ...
            adverse_events_value = record.get("adverse_events") if record else None
            interactions_value = record.get("interactions_structured") if record else None
            safety_findings_text = record.get("safety_findings_text") if record else None
            interaction_text = record.get("interaction_text") if record else None
            safety_findings = ""
            interactions = ""
            safety_reassurance = ""
            ...
            if record:
                ...
                if adverse_events_value is not None or interactions_value is not None:
                    structured_result = extract_structured_safety_interactions(
                        adverse_events_value, interactions_value, plant_name=plant,
                    )
                ...
                text_result = extract_structured_safety_interactions(
                    safety_findings_text, interaction_text, plant_name=plant,
                )
                structured_safety = _merge_unique_text([
                    structured_safety, "; ".join(text_result["adverse_events"])
                ])
                structured_interactions = _merge_unique_text([
                    structured_interactions, "; ".join(text_result["interactions"])
                ])
                structured_reassurance = _merge_unique_text([
                    structured_reassurance, "; ".join(text_result["safety_reassurance"])
                ])
