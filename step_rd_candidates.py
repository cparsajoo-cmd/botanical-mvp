...
def _run_evidence_adjudication(plant_summary_df, evidence_df, indication, target_context):
    ...
    for idx, row in plant_summary_df.iterrows():
        ...
        new_columns["AI_Direct_Outcome_Evidence_Count"][idx] = len(_as_id_list(adjudication.get("Direct_Outcome_Evidence_IDs")))
        new_columns["AI_Direct_Human_Outcome_Evidence_Count"][idx] = len(_as_id_list(adjudication.get("Direct_Human_Outcome_Evidence_IDs")))
...
def _pipeline_implementation_fingerprint() -> str:
    """Return a deterministic fingerprint of the code that defines Stage 5/6.

    Step 5 deliberately reuses completed results when the user clicks Run again
    with identical scientific inputs.  Before this fingerprint existed, that
    reuse key only contained a manually-maintained constant.  A hot-reloaded
    algorithm patch could therefore keep serving an old ``rd_candidates_df``
    even though the source files on disk had changed.  Hashing the small set of
    modules that actually define candidate generation, prescreening, scoring,
    R&D-lane classification and compound resolution makes code changes
    invalidate both the per-session result reuse key and the cached engine
    automatically.  Missing files are hashed as explicit sentinels rather than
    silently ignored, so the fingerprint is deterministic in partial installs.
    """
    root = Path(__file__).resolve().parent
    digest = hashlib.sha256()
    for filename in _PIPELINE_FINGERPRINT_FILES:
...
def _build_engine(evidence_df, use_live_search, discovered_candidates=None):
    ...
    return _cached_engine(
        use_live_search,
        fingerprint,
        f"{ENGINE_CACHE_VERSION}:{_pipeline_implementation_fingerprint()}",
...
def _candidate_discovery_run_key(
    *, indication, dosage_form, market, reference_plant, reference_compound,
    ...
    return (
        ...
        _pipeline_implementation_fingerprint(),
    )
...
def _stage6_stale_pipeline_warning(session_report_ready_df) -> str | None:
    """Real production freshness guard for the Stage 6 call site.

    Pure (no Streamlit calls) so it is directly testable. Returns the warning
    text to show (and, implicitly, "do not render") when the REAL
    st.session_state["rd_report_ready_df"] is non-empty but either carries no
    Pipeline_Implementation_Fingerprint column at all (older sessions never
    wrote one -- treated as stale here, unlike a synthetic/unit-test frame
    passed directly to _recommendation_block()) or its fingerprint(s)
    disagree with the current code. Returns None when there is nothing to
    block on (no report-ready frame yet, or it matches the current pipeline).
    """
...
def _recommendation_block(
    ...
    #
    # NOTE: this function is a presentation/rendering boundary only. It does
    # NOT validate pipeline-implementation freshness -- that check requires a
    # production-vs-stale-session judgment call (e.g. "no fingerprint at all
    # counts as stale") that only makes sense at the real Stage 6 call site,
    # where the frame actually comes from st.session_state["rd_report_ready_
    # df"]. A direct/unit-test call always receives a caller-constructed
    # DataFrame -- possibly with no fingerprint at all -- and must render it.
    # See _report_ready_matches_current_pipeline() and its call site in
...
def render_rd_candidates_step(inputs):
        ...
        if (
            ...
            try:
                ...
                if isinstance(result_df, pd.DataFrame) and not result_df.empty:
                    # Diagnostic provenance for exports/reviews.  This is not a
                    # scientific feature and never participates in scoring.  It
                ...
                if isinstance(result_df, pd.DataFrame) and not result_df.empty:
                    ...
                    st.session_state["rd_pipeline_implementation_fingerprint"] = pipeline_fingerprint
    ...
    if st.session_state.get("show_final_recommendation"):
        _session_report_ready_df = st.session_state.get("rd_report_ready_df")
        _stale_warning = _stage6_stale_pipeline_warning(_session_report_ready_df)
        if _stale_warning:
