...
- **Superseded (not abandoned, but not canonical) — Case 008's original draft.** `gold_case_reference_grounded_008_ginkgo_biloba_indicationevidence.py` (domain INDICATION_EVIDENCE, hard-coded per `CASES_008_009_010_012_CORRECTION_REPORT.md`) was replaced 2026-08-01 by `gold_case_reference_grounded_008_ginkgo_biloba_preparation_spec.py` (domain PREPARATION_SPEC). Unlike Case 002, the superseded draft is retained for audit history, but as of 2026-08-07 it and its old test have been moved under `gold_cases/archive/superseded_case_008/` and renamed with a `superseded_` prefix. It is no longer a live import/discovery risk alongside the canonical Case 008. See closed NA-010 in `NEXT_ACTIONS.md`.
