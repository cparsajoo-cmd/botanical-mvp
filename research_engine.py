...
def _extract_open_world_botanical_candidates(
    ...
    Two PARALLEL, independent extraction paths feed the same mention pool
    below: the existing high-precision regex path
    (extract_binomial_mentions -- literal "Genus species" text only) and
    an additional high-recall AI path
    (ai_botanical_entity_extractor.extract_botanical_entities_ai --
    common names, synonyms, abbreviated binomials). Both paths only ever
    produce STRING PROPOSALS; every proposal from either path is
    validated by the exact same validate_botanical_candidate() call in
    Pass 2 below -- the AI path has no taxonomic authority and cannot
    bypass this check (see ai_botanical_entity_extractor.py's module
    docstring). ``max_llm_entity_extraction_records`` bounds how many
    indication-relevant records the AI path runs on per call (cost
    control -- see Part 19 of the architecture spec); the regex path is
    ...
    # AI entity extraction runs at most once per record (never once per
    # mention), and is capped to the first N indication-relevant records
    # so a large literature pull cannot generate an unbounded number of
    # LLM calls. Cached here by record index and reused identically in
    # Pass 1 (mention collection) and Pass 3 (per-record scoring) below,
    ...
    # mention across all indication-relevant records, from BOTH paths.
    mention_display = {}
    for index, record in enumerate(records):
        raw_title = record.get("Title") or record.get("Source_Title") or ""
        raw_abstract = record.get("Abstract") or record.get("Notes") or record.get("Raw_Text") or ""
        ...
        for ai_candidate in ai_candidates_by_index.get(index, []):
            mention_norm = _norm_text(ai_candidate)
            if mention_norm in catalogue_norms:
                continue
            mention_display.setdefault(mention_norm, ai_candidate)
    ...
    # including when one path (regex) and the other (AI) found the SAME
    # taxon via different original mentions in the same or different
    # records; canonical_sources below tracks which discovery path(s)
    # actually contributed to each canonical candidate.
    scores = defaultdict(float)
    supports = defaultdict(set)
    title_supports = defaultdict(set)
    clinical_supports = defaultdict(set)
    ...
    for index, record in enumerate(records):
        ...
        def _apply_mention(mention_norm, in_title, discovery_source):
            if mention_norm not in validated:
                return
            canonical = validated[mention_norm]["matched_scientific_name"] or mention_norm
            canonical_sources[canonical].add(discovery_source)
        ...
        for raw_text, in_title in ((raw_title, True), (raw_abstract, False)):
            for mention in extract_binomial_mentions(raw_text):
                _apply_mention(_norm_text(mention), in_title, "regex_open_world")

        for ai_candidate in ai_candidates_by_index.get(index, []):
            # AI candidates are derived from title+abstract jointly (see
            # ai_botanical_entity_extractor.py); they are not separately
            # attributable to title vs abstract, so in_title=False here
            # only affects the title_supports bonus, never whether the
            # candidate is counted at all.
            _apply_mention(_norm_text(ai_candidate), False, "llm_open_world")
    ...
    match_diagnostics = {
        plant: {
            ...
            # Additive provenance (Part 6): which discovery path(s) found
            # this canonical candidate. Never replaces candidate_origin
            # above, which is preserved unchanged for backward
            # compatibility.
            "candidate_sources": sorted(canonical_sources.get(plant, set())),
        }
        for plant in ranked
    }
    ...
    novel_discovered_candidates = [
        {
            ...
            # Additive provenance (Part 6) -- which open-world discovery
            # path(s) found this candidate (regex_open_world /
            # llm_open_world, or both). candidate_origin above is kept
            # unchanged for backward compatibility.
            "candidate_sources": novel_meta.get(plant, {}).get("candidate_sources", []),
        }
        for plant in novel_candidates
    ]
