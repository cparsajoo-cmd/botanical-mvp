...
# get_openai_client is now implemented once, centrally, in llm_client.py
# (API key resolution, Streamlit-secrets fallback, model config, caching,
# structured-call helper). Re-exported under its original name here --
# rather than only imported for internal use -- because at least one
# existing test (test_llm_extractor_gate_assertion_prompt.py) monkeypatches
